...
- **14 Sep 2026 (lanjutan)** — **UX Rounds 6-8 (konten aset + logo final + AI chain):** `MachinesSection` baru (AccurioPress C2060 + bizhub 958 + foto toko asli — trust "mesin industri beneran"), Portfolio ganti 7 SVG placeholder → 5 poster brand dari archive + CTA tile (`lg:grid-cols-3`), OG image rebuild `og-image.png` 1200×630 branded, **logo fix final**: root cause = `next/image` optimizer serve PNG stale → rename `logo-bisaprint-mark-v2.png` (cache-bust) + `translate-y-[2px]` (massa huruf BP sejajar baseline wordmark — verified 5×/1×), **`/api/ai-design` chain OpenAI → Gemini (`GEMINI_API_KEY`, gratis AI Studio) → Pollinations** — fallback bertingkat, app tak pernah mati tanpa key. Cleanup: 8 duplikat archive↔public dihapus, 0 orphan `public/`, 0 secret tracked. 161/161 tests, tsc/eslint/build hijau, route health live-verified (`/` `/simulator` `/checkout` 200, admin 401 fail-closed, POST `/api/ai-design` 200). Commits `af86f1b`→`2a663ea`.
