...
| production-dashboard-flow | Supporting | Done (v2) | Done (v2) | Done | In Progress | N/A | N/A | N/A | IN PROGRESS | 2026-09-12 | Track A v2 Tahap 0–2: dashboard dibangun — `/admin/orders` (SSR + client pagination/status), `GET /api/admin/orders` + `PATCH /api/admin/orders/[id]` (Zod), Basic Auth di `proxy.ts` + re-check per-handler (fail-closed, `ADMIN_USERNAME`/`ADMIN_PASSWORD`), `listOrders` + `production.status` terpisah dari webhook state machine. PD-A-01/03/04 FIXED, PD-A-02 FIXED, PD-A-05 ACK. 139/139 tests, live-verified 401. Non-blocking: isi `ADMIN_*` + `ADMIN_NOTIFY_WEBHOOK_URL` untuk operasional penuh. Tahap 3 menyusul. |
